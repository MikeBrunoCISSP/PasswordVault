...
public class crypto
{
    ...
    public crypto(string keyMaterial, int iterations)
    {
        hm = new HashManager("SHA256", iterations);
        key = keyMaterial + flip(keyMaterial);
        /*saltAndIV = GeneratePassword(24, 24);
        salt = stdlib.Left(saltAndIV, 8);
        initializationVector = stdlib.Right(saltAndIV, 16); */
    ...
    /*public crypto(string keyMaterial, string saltAndInitializationVector, int iterations)
    {
        key = keyMaterial + flip(keyMaterial);
        saltAndIV = saltAndInitializationVector;
        salt = stdlib.Left(saltAndIV, 8);
        initializationVector = stdlib.Right(saltAndIV, 16);
        i = iterations;
    } */
...
class HashManager
{
    ...
    public HashManager(string hashAlgorithm, int iterations)
    {
        ...
        i = iterations;
        log = new Log(@"C:\hashmanager.log", INFO);
    }
    #endregion
    ...
    {
        log.write(DEBUG, "Entering method \"compute\"", false);
        if ((message == null) || (message == ""))
            return null;

        log.write(DEBUG, "Message: " + message, false);
        byte[] salt = getBytes(crypto.GeneratePassword(4, 8));
        log.write(DEBUG, "Salt value: " + ASCIIEncoding.ASCII.GetString(salt), false);

        byte[] messageBytes = getBytes(message);
        byte[] messageBytesWithSalt = concatenate(messageBytes, salt);
        log.write(DEBUG, "Message with salt appended: " + ASCIIEncoding.ASCII.GetString(messageBytesWithSalt), false);
        byte[] hashValue = messageBytes;

        for (int x = 0; x < i; x++)
        {
            log.write(DEBUG, "Hash iteration " + x + " of " + (i - 1).ToString(), false);
            log.write(DEBUG, "Current Message: " + Convert.ToBase64String(messageBytesWithSalt), false);
            hashValue = hash.ComputeHash(messageBytesWithSalt);
            log.write(DEBUG, "Computed Hash: " + Convert.ToBase64String(hashValue), false);
        ...
        log.write(DEBUG, "Hash value being returned: " + Convert.ToBase64String(concatenate(hashValue, salt)) + Environment.NewLine, false);
        return Convert.ToBase64String(concatenate(hashValue,salt));
    }

    private string computeWithKnownSalt(string message, string salt)
    {
        log.write(DEBUG, "Entering method \"computeWithKnownSalt\"", false);
        log.write(DEBUG, "Message: " + message, false);
        log.write(DEBUG, "Salt: " + salt, false);
        if ((message == null) || (message == ""))
            return null;
        ...
        log.write(DEBUG, "Concatenated Message to be hashed: " + Convert.ToBase64String(messageBytesWithSalt), false);
        byte[] hashValue = messageBytes;
        ...
        log.write(DEBUG, "Hash value being returned: " + Convert.ToBase64String(concatenate(hashValue, saltBytes)) + Environment.NewLine, false);
    ...
    {
        log.write(DEBUG, "Entering method \"verify\"", false);
        log.write(DEBUG, "Message: " + message, false);
        log.write(DEBUG, "Expected Hash Value: " + digest, false);
        if (digest.Length < hashSize)
            return false;

        if (digest.Length > hashSize)
        {
            log.write(DEBUG, "Salt detected...", false);
            byte[] saltBytes = getSalt(digest);
            string salt = ASCIIEncoding.ASCII.GetString(saltBytes);
            log.write(DEBUG, "Salt value: " + salt, false);
    ...
    {
        log.write(DEBUG, "Entering method \"verifyWithSalt\"", false);
        log.write(DEBUG, "Message: " + message, false);
        log.write(DEBUG, "Salt: " + salt, false);
        log.write(DEBUG, "Expected Hash: " + expectedDigest, false);
        log.write(DEBUG, "Computing actual hash...", false);
        string actualDigest = computeWithKnownSalt(message, salt);
        log.write(DEBUG, "Actual Hash Value: " + actualDigest, false);
        return (expectedDigest == actualDigest);
        /*byte[] plainTextBytes = getBytes(plaintext);
        byte[] plainTextBytesWithSalt = concatenate(plainTextBytes, getBytes(salt));
        byte[] expectedHash = plainTextBytes;

        for (int x = 0; x < i; x++)
        {
            expectedHash = hash.ComputeHash(plainTextBytesWithSalt);
            plainTextBytesWithSalt = expectedHash;
        }
